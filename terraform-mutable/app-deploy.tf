resource "null_resource" "app-deploy" {
  ...
  provisioner "remote-exec" {
    inline = [
      "ansible-pull -U https://github.com/siddhudeva/ansible-1.git roboshop-pull.yml -e COMPONENT=${var.COMPONENT} -e ENV=${var.ENV} -e APP_VERSION=${var.APP_VERSION} -e NEXUS_USERNAME = local.NEXUS_USERNAME -e NEXUS_PASSWD = local.NEXUS_PASSWD"
